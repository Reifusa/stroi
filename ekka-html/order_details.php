...
if (!isset($_GET['order_id']) || empty($_GET['order_id'])) {
    die("Ошибка: не указан идентификатор заказа.");
}

// Получаем ID заказа
$order_id = intval($_GET['order_id']);

// SQL-запрос для получения деталей заказа
$query = "
    SELECT 
        oi.prod_id,
        oi.unit_price,
        oi.quanty AS quantity,
                ...
                <div class="col-12">
                    <div class="row ec_breadcrumb_inner">
                        <div class="col-md-6 col-sm-12">
                            <h2 class="ec-breadcrumb-title">Детали заказа</h2>
                        </div>
                        <div class="col-md-6 col-sm-12">
                            <!-- ec-breadcrumb-list start -->
                            <ul class="ec-breadcrumb-list">
                                <li class="ec-breadcrumb-item"><a href="index.html">Главная</a></li>
                                <li class="ec-breadcrumb-item"><a href="order-add.php">Заказы</a></li>
                                <li class="ec-breadcrumb-item active">Детали заказа</li>
                            </ul>
...
// Подготовка запроса для получения статуса заказа
...
$can_change_status = ($status !== 'Закрыт' && $status !== 'Открыт');
?>

<!-- Формы для подтверждения и отмены -->
                            ...
                            <div class="ec-vendor-card-table">
                                <table class="table ec-table">
                                    ...
                                    if ($result->num_rows === 0) {
                                        echo "<p>Детали заказа не найдены.</p>";
                                        exit();
                                    }

                                    // Выводим детали заказа
                                    echo '<tbody class="wish-empt">';
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $total_price = number_format($row['unit_price'] * $row['quantity'], 2, '.', '');
                                        $product_url = "product-full-width.php?id=" . intval($row['prod_id']); // Ссылка на страницу товара

                                        // Формируем строку таблицы
                                        echo "
        <tr class='pro-gl-content'>
            <td><img class='prod-img' src='../img/{$product_image}' alt='product image'></td>
            ...
            <td>
                <a href='{$product_url}' class='btn btn-primary'>
                    <img src='assets/images/icons/search.svg' class='svg_img header_svg' alt='Искать' style='width: 15px; text-align: center' />
                </a>
            </td>
        </tr>";
                                    }
